...
class odomTransfer:
    ...
    def change2MapPixels(self, searchPoints):
        ...
        for i in range(len(searchPoints)):
            x = searchPoints[i][0] / 200.0 * self.fullMapCols + self.fullMapCols/2.0
            y = -searchPoints[i][1] / 200.0 * self.fullMapRows + self.fullMapRows/2.0

            #ROS_INFO("realx:%f, realy:%f \n", realx, realy);
            realx =  int(x - self.leftBound);
            realy =  int(y - self.upBound);
