...
public:
      ...
      :it_(nh_) //构造函数
    {
        ...
        while(image_sub_.getNumPublishers() < 1){
            ROS_INFO("wait for cv_camera node. Publisher:%d", image_sub_.getNumPublishers());
            loop_rate.sleep();
            image_sub_ = it_.subscribe("cv_camera/image_raw", 1, &RGB_GRAY::convert_callback, this); //定义图象接受器，订阅话题是“camera/rgb/image_raw”
        ...
        while(command_sub.getNumPublishers() < 1){
            ROS_INFO("wait for dn_finder node. Publisher:%d", command_sub.getNumPublishers() );
            loop_rate.sleep();
            command_sub = nh_.subscribe("dn_finder_command", 10, &RGB_GRAY::sub_dn_finder_command, this);
