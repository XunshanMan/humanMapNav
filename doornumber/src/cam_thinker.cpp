...
#define PIC_SAVE_LOCATIONS "/home/jk/catkin_lzw/src/doornumber/debug/%04d.jpg"
#define PIC_SAVE_LOCATIONS_ORIGIN "/home/jk/catkin_lzw/src/doornumber/debug/%04d_origin.jpg"
...
char filename_origin[1000][10] = {0};
...
public:
      ...
      :it_(nh_) //构造函数
    {
         ...
         * For the topic publisher initialize
         */
        doornumber_pub = nh_.advertise<doornumber::boardArray>("boardArray", 1000);
        ...
        command_sub = nh_.subscribe("dn_finder_command", 10, &RGB_GRAY::sub_dn_finder_command, this);
    ...
    */
    void image_process(cv::Mat img)
    {
       ...
       case 1: //multioriented
           erGrouping(frame, channels, regions, nm_region_groups, nm_boxes, ERGROUPING_ORIENTATION_ANY, "/home/jk/catkin_lzw/src/doornumber/xml/trained_classifier_erGrouping.xml", 0.5);
           break;
       }
       ...
       for (int i=0; i<(int)detections.size(); i++)
       {
           ...
           for (int j=0; j<(int)boxes[i].size(); j++)
           {
               ...
               //change tlx to be the proportion of the position in the pic
               newboard.tlx = (boxes[i][j].x - frame.cols/2) / frame.cols;
       ...
       if(CaptureNum < 999)
            CaptureNum++;
       sprintf(filename[CaptureNum], PIC_SAVE_LOCATIONS,CaptureNum);//保存的格式为.bmp,当然可以自己设定为jpg等
        imwrite(filename[CaptureNum],out_img);
        sprintf(filename_origin[CaptureNum], PIC_SAVE_LOCATIONS_ORIGIN,CaptureNum);//保存的格式为.bmp,当然可以自己设定为jpg等

        imwrite(filename_origin[CaptureNum], frame);
...
int main(int argc, char** argv)
{
    ...
    for (int i=0; i<2; i++)
    {
        Ptr<ERFilter> er_filter1 = createERFilterNM1(loadClassifierNM1("/home/jk/catkin_lzw/src/doornumber/xml/trained_classifierNM1.xml"),8,0.00015f,0.13f,0.2f,true,0.1f);
        Ptr<ERFilter> er_filter2 = createERFilterNM2(loadClassifierNM2("/home/jk/catkin_lzw/src/doornumber/xml/trained_classifierNM2.xml"),0.5);
    ...
    for (int o=0; o<num_ocrs; o++)
    {
        ...
        ocrs[o]->setWhiteList("L0123456789-");
    }
    Mat transition_p;
    string filename = "/home/jk/catkin_lzw/src/doornumber/xml/OCRHMM_transitions_table.xml";
    ...
    for (int o=0; o<num_ocrs; o++)
    {
        decoders.push_back(OCRHMMDecoder::create(loadOCRHMMClassifierNM("/home/jk/catkin_lzw/src/doornumber/xml/OCRHMM_knn_model_data.xml.gz"),
