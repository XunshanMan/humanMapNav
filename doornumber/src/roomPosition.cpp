...
public:
    ...
    void initialROS(){
        int argc = 0;
        char** argv = 0;
        ros::init(argc, argv, "roomPosition");

        Point_pub = nh_.advertise<std_msgs::Float32MultiArray>("roomPosition", 1);
        roomnumber_sub = nh_.subscribe("roomNumber", 1, subRoomNumber);
     ...
     * For ros system
     */
    void publish(    std_msgs::Float32MultiArray data){
        Point_pub.publish(data);
    }

    void subRoomNumber(const std_msgs::String::ConstPtr& msg)
    {
        //Point t = AlphaDog::getDestinationOnCorridor(msg->data);
        Point t;

        /*   The part for get the OCR CENTER */
        Point ocr_center = getRoomCenter(msg->data);
        if(ocr_center.x == 99999 && ocr_center.y == 99999)
            t = Point(99999, 99999);
            // wrong value
        Mat image;
        ...
        floodFillRoom(ocr_center);

        namedWindow("g_dstImage", WINDOW_NORMAL);
        ...
        std_msgs::Float32MultiArray pubdata;
        pubdata.data.push_back(t.x);
        pubdata.data.push_back(t.y);

        Point_pub.publish(pubdata);
    ...
    void floodFillRoom(Point center){
        ...
        //随机生成bgr值
        int b = 0;//随机返回一个0~255之间的值
        int g = 0;//随机返回一个0~255之间的值
        ...
        else
        {
            area = floodFill(dst, seed, newVal, &ccomp, Scalar(LowDifference, LowDifference, LowDifference),
                Scalar(UpDifference, UpDifference, UpDifference), flags);
            area = floodFill(dst, seed2, newVal, &ccomp, Scalar(LowDifference, LowDifference, LowDifference),
                Scalar(UpDifference, UpDifference, UpDifference), flags);
            area = floodFill(dst, seed3, newVal, &ccomp, Scalar(LowDifference, LowDifference, LowDifference),
                Scalar(UpDifference, UpDifference, UpDifference), flags);
        }
        cout << area << " Points were redrawn.\n";
    ...
    /*
     * This Function is for getting room pixel position.
     * ( which is the center of the OCR_TEXT )
     *
     */
    Point getRoomCenter(String roomNumber){
        Point ocr_center;
        for (int i=0; i<boards.boardArray.size(); ++i)
        {
            const doornumber::board &board = boards.boardArray[i];
            //ROS_INFO_STREAM("text: " << board.text << "confidence: " << board.confidence);
            string::size_type idx;
            idx=board.text.find(roomNumber);//在a中查找b.

            if(idx != string::npos){
                ROS_INFO("Final Goal is %s [Confidence %.1f]", board.text.c_str(), board.confidence);
                ocr_center.x = (board.tlx + board.brx) /2;
                ocr_center.y = (board.tly + board.bry) /2;
                return ocr_center;
                break;
            }
    ...
    Point getDestinationOnCorridor(String roomNumber){
        Point ocr_center = getRoomCenter(roomNumber);
        if(ocr_center.x == 99999 && ocr_center.y == 99999)
            return Point(99999, 99999);
            // wrong value
        Mat image;
...
int main(int argc, char** argv)
{
    ...
    //obj.getRoomAreas();
        ...
        // first return a little point.
        // tomorrow finish this part ok?
